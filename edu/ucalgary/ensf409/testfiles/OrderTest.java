...
public class OrderTest {
    ...
    private ArrayList<Family> families = new ArrayList<>(new Family(1, 3, 2, 4, 1);
    ...
    public void testOrderClassConstructor(){
        //needs more test
        ArrayList<Family> testFam = order.getFamilies();

        boolean familiesEqual = true;

        for (int i = 0; i < 5; i++){
            if (families[i] != testFam.get(i)){
                familiesEqual = false;
                break;
            }
        }
        ...
        assertTrue("The instructor doesnt transfere Objcts from array " +
                "to the ArrayList properly", familiesEqual);
    }
...
//    }
    ...
    public void testGetTotalFoodItemsArrayList(){
        ...
        Order testOrder = new Order(testFamily);
        ArrayList<Food> realTotalFoodItems = order.getTotalFoodItems();

        for (int i = 0; i < realTotalFoodItems.size(); i++){
            if (realTotalFoodItems.get(i) != expectedFoodList.get(i)){
                equalItems = false;
                break;
            }
        }

        assertEquals("Items are not set properly on the constructor!",
                equalItems);
